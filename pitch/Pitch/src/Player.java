...
public class Player {
	...
	public void fillBidderHand(Deck deck, int suitIdx){
		...
		while(deck.getIterator() < 54){
			...
			if( next.getSuit() == suitIdx || next.offJack(suitIdx) || next.getSuitName().equals("Joker")){
				hand.addCard(next);
			}
			else{
				temp.add(next);
	...
	public Card playCard(){
		...
		} while (cardPlayedIdx < 0 || cardPlayedIdx > this.getHand().size() || this.getHand().size() <=0);
		Card cardPlayed = hand.getHand().get(cardPlayedIdx);
